    ...
    <form action="index.php" method="post" enctype="multipart/form-data" id="multiForm">
        ...
        <div class="card hidden" id="2">
            ...
            <div>
                ...
                <div class="radio-group">
                    ...
                    <label><input type="radio" name="civilite" value="Mme"
                            <?php if(isset($civilite) && $civilite === 'Mme') echo 'checked'; ?>> Femme</label>
                    <label><input type="radio" name="civilite" value="Autre"
                            <?php if(isset($civilite) && $civilite === 'Autre') echo 'checked'; ?>> Autre</label>
                </div>
            </div>
            ...
            <div class="inline-flex">
                <div class="next-btn" role="group" aria-label="Précédent action">
                    ...
                    <button type="button" class="arrow-only" aria-label="Précédent" onclick="showPreviousCard()">
                        <img src="../../img/svg/fleche-gauche.svg" alt="Précédent"
                            style="filter : invert(1) saturate(0.9)" class="btn-arrow-left" aria-hidden="true">
                    </button>
                </div>
                <div class="next-btn" role="group" aria-label="Suivant action">
                    <span class="next-text">Suivant</span>
        ...
        <div class="card hidden" id="3">
            ...
            <div>
                ...
                <input type="text" id="compelement" name="complement" placeholder="ex: appartement 207"
    ...
    <div id="cgvModal" class="modal hidden">
        <div class="modal-content">
            ...
            <div class="cgv-content">
                ...
                <p>Alizon est un marketplace proposant à des tiers de mettre en avant des produits ainsi que les
                    vendres. L’utilisation du service implique l’<strong>acceptation des présentes CGU.</strong></p>
                <h3>ARTICLE 2 : COMPTE UTILISATEUR</h3>
                <p>Vous êtes <strong>responsable de la confidentialité</strong> de vos identifiants et mots de passe.
                    Vous <strong>garantissez l’exactitude</strong> des informations fournies.</p>
                <h3>ARTICLE 3 : CONTENU</h3>
                <p><strong>Interdiction</strong> de publier des contenus illégaux, haineux, diffamatoires,
                    contrefaisants ou nuisibles. Le contenu que vous publiez <strong>reste sous votre
                        responsabilité.</strong></p>
                <h3>ARTICLE 4 : PROPRIÉTÉ INTELLECTUELLE</h3>
                <p><em>Vos droits :</em></p>
                <p>Vous <strong>conservez vos droits</strong> sur vos contenus. Vous nous accordez une <strong>licence
                        limitée</strong> pour l’affichage et l’hébergement nécessaires à la fourniture du service. Nous
                    nous engageons à respecter vos droits et à ne pas utiliser vos contenus à d’autres fins que celles
                    prévues par le service.</p>
                <p><em>Droits de Alizon :</em></p>
                <p>Le <strong>code source</strong> de Alizon appartient exclusivement à la SCOP SARL COBREC, all rights
                    reserved (tous droits réservés). Les autres éléments du site (textes, images, logos, icônes) sont la
                    propriété de la SCOP SARL COBREC ou de ses partenaires et sont protégés. <strong>Toute reproduction
                        non autorisée est interdite</strong>. Pour toute demande de licence ou de réutilisation,
                    contactez-nous à contact@alizon.bzh.</p>
                <p><em>Engagement de l'utilisateur :</em></p>
                <p>L'utilisateur </strong>s'engage à respecter les droits de propriété intellectuelle</strong> (droit
                    d'auteur, droits voisins du droit d’auteur, droit des marques et des logos). Il s'engage à
                    n'utiliser que des images ou autres éléments dont il <strong>détient légalement les droits</strong>
                    ou pour lesquels il dispose d’une autorisation expresse des titulaires.</p>
                <h3>ARTICLE 5 : SUSPENSION ET RÉSILIATION</h3>
                <p>Nous pouvons suspendre ou résilier l’accès en cas de non-respect manifeste des CGU, de fraude ou
                    d’atteinte à la sécurité.</p>
                <h3>ARTICLE 6 : RESPONSABILITÉ</h3>
                <p>Le service est fourni « <strong>en l’état</strong> ». Nous ne garantissons pas une disponibilité
                    ininterrompue. Notre responsabilité ne saurait être engagée pour des <strong>dommages
                        indirects.</strong></p>
                <h3>ARTICLE 7 : DONNÉES PERSONNELLES</h3>
                <p>Le traitement des données est décrit dans les <strong>mentions légales</strong>. En vous inscrivant
                    sur Alizon, vous acceptez expressément ces traitements et confirmez avoir pris connaissance des
                    mentions légales.</p>
                <h3>ARTICLE 8 : MODIFICATIONS</h3>
                <p>Nous pouvons modifier les présentes CGU. Vous serez automatiquement informé de chaque modification
                    des CGU par une pop-up lors de votre connexion. Les modifications sont applicables dès leur
                    publication. La date de mise à jour est indiquée en entête.</p>
        ...
        if (visible === 0) {
            ...
            if (pseudoInput && pseudoInput.value.trim()) {
                ...
                if (pseudoResult.exists) {
                    if (errorEl) {
                        errorEl.classList.remove('hidden');
                        errorEl.innerHTML = '<strong>Erreur</strong> : ' + (pseudoResult.message ||
        ...
        if (visible === 1) {
            ...
            if (emailInput && emailInput.value.trim()) {
                ...
                if (emailResult.exists) {
                    if (errorEl) {
                        errorEl.classList.remove('hidden');
                        errorEl.innerHTML = '<strong>Erreur</strong> : ' + (emailResult.message ||
            ...
            if (phoneInput && phoneInput.value.trim()) {
                ...
                if (phoneResult.exists) {
                    if (errorEl) {
                        errorEl.classList.remove('hidden');
                        errorEl.innerHTML = '<strong>Erreur</strong> : ' + (phoneResult.message ||
