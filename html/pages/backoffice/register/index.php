...
<?php
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $Cmdp = $_POST['Cmdp'] ?? '';
      ...
      if (!$hasError && !preg_match('/^(?:\d{9}|\d{3} \d{3} \d{3})$/', $SIREN) ) {
        $hasError = true;
        $error_card = 1; 
        $error_message = 'Veuillez saisir un SIREN valide (9 chiffres ou format "123 456 789").';
      }
    ...
    if (!$hasError) {
      ...
      echo "<dt>Raison sociale</dt><dd>{$Rsociale}</dd>";
      echo "<dt>nuéro de SIREN</dt><dd>{$SIREN}</dd>";
  ...
  <img src="../../../img/svg/logo-text.svg" alt="Logo Alizon">
    ...
    <p class="subtitle">Identifiants</p>
      ...
      <div>
        ...
        <input type="text" id="Rsociale" name="Rsociale" placeholder="Votre Raison sociale" required>
      </div>
    ...
    <p class="subtitle">Coordonnées</p>
      ...
      <div class="inline-flex address-row">
        <div class="culumn-flex" id="div_codeP">
          ...
          <input type="text" id="codeP" name="codeP" inputmode="numeric" pattern="^[0-9]{5}$" maxlength="5" placeholder="ex: 22300" required title="Le code postal doit contenir 5 chiffres">
        </div>
