...
$listeProduitsTous = ProduitDenominationVendeur($connexionBaseDeDonnees, $vendeurInfos['denomination']);

// Filtre les produits qui sont soit en promotion
$listeProduits = array_filter($listeProduitsTous, function($p) {
    $enPromo = !empty($p['estenpromo']);
    return $enPromo;
});
...
<body>
    <header>
        <h1>Catalogue des Produits en Promotion de <?php echo htmlspecialchars($vendeurInfos['denomination'] ?? ''); ?>
        </h1>
    </header>

    <main>
        <!--affiche un message si le vendeur n'a aucun produit en promotion-->
        <?php if (empty($listeProduits)): ?>
        <p>Ce vendeur n'a aucun produit en promotion ou réduction pour le moment.</p>
        <?php else: ?>
